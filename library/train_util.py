...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def is_text_encoder_output_cacheable(self, cache_supports_dropout=False):
        return all(
            [
                not (
                    (not cache_supports_dropout and subset.caption_dropout_rate > 0)
                    or subset.shuffle_caption
                    or subset.token_warmup_step > 0
                    or (not cache_supports_dropout and subset.caption_tag_dropout_rate > 0)
                )
    ...
    def new_cache_text_encoder_outputs(self, models: List[Any], accelerator: Accelerator):
        ...
        # propagate subset-level caption_dropout_rate to each ImageInfo
        for info in image_infos:
            subset = self.image_to_subset.get(info.image_key)
            if subset is not None and hasattr(subset, "caption_dropout_rate"):
...
class DatasetGroup(torch.utils.data.ConcatDataset):
    ...
    def is_text_encoder_output_cacheable(self, cache_supports_dropout=False) -> bool:
        return all([dataset.is_text_encoder_output_cacheable(cache_supports_dropout=cache_supports_dropout) for dataset in self.datasets])
