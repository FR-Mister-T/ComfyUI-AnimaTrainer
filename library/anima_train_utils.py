...
from .device_utils import init_ipex, clean_memory_on_device, synchronize_device
from . import anima_models, anima_utils, train_util, qwen_image_autoencoder_kl
...
def compute_loss_weighting_for_anima(weighting_scheme: str, sigmas: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
    """Compute loss weighting for Anima training.

    Returns None when sigmas is None (uniform/sigmoid/shift/flux_shift timestep sampling),
    which signals the training loop to apply no per-sample weighting.
    """
    if sigmas is None:
        return None
...
def sample_images(
    ...
    # Parse prompts: args.sample_prompts is a list of strings (from ComfyUI) or a file path
    if isinstance(args.sample_prompts, list):
        raw = [p for p in args.sample_prompts if p.strip() and p[0] != "#"]
        prompts = []
        for i, p in enumerate(raw):
            d = train_util.line_to_prompt_dict(p) if isinstance(p, str) else p
            d["enum"] = i
            d.pop("subset", None)
            prompts.append(d)
    elif os.path.isfile(args.sample_prompts):
        prompts = train_util.load_prompts(args.sample_prompts)
    elif sample_prompts_te_outputs is None:
        logger.error(f"No prompt file: {args.sample_prompts}")
        return
    else:
        prompts = []

    if not prompts and sample_prompts_te_outputs is None:
        return
    ...
    with torch.no_grad(), accelerator.autocast():
        for prompt_dict in prompts:
            # Apply validation_settings overrides (from AnimaTrainValidationSettings node)
            if validation_settings:
                merged = dict(prompt_dict)
                if "width" in validation_settings:
                    merged["width"] = validation_settings["width"]
                if "height" in validation_settings:
                    merged["height"] = validation_settings["height"]
                if "guidance_scale" in validation_settings:
                    merged["scale"] = validation_settings["guidance_scale"]
                if "flow_shift" in validation_settings:
                    merged["flow_shift"] = validation_settings["flow_shift"]
                if "seed" in validation_settings:
                    merged["seed"] = validation_settings["seed"]
                if "steps" in validation_settings:
                    merged["sample_steps"] = validation_settings["steps"]
                prompt_dict = merged
