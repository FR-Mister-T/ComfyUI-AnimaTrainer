...
def swap_weight_devices_no_cuda(device: torch.device, layer_to_cpu: nn.Module, layer_to_cuda: nn.Module):
    ...
    # device to cpu — blocking to avoid race conditions on ROCm
    for module_to_cpu, module_to_cuda, cuda_data_view, cpu_data_view in weight_swap_jobs:
        module_to_cpu.weight.data = cuda_data_view.data.to("cpu")

    synchronize_device(device)

    # cpu to device — blocking to avoid race conditions on ROCm
    for module_to_cpu, module_to_cuda, cuda_data_view, cpu_data_view in weight_swap_jobs:
        cuda_data_view.copy_(module_to_cuda.weight.data)
        module_to_cuda.weight.data = cuda_data_view
...
class Offloader:
    ...
    def __init__(self, num_blocks: int, blocks_to_swap: int, device: torch.device, debug: bool = False):
        ...
        self.futures = {}
        # ROCm exposes device.type=="cuda" but torch.cuda streams from background
        # threads cause GPU page faults on ROCm — use the synchronous path instead.
        self.cuda_available = device.type == "cuda" and not _is_rocm()
