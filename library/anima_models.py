...
class FinalLayer(nn.Module):
    ...
    def forward(
        ...
        # When use_fp32 is False (bf16 training), use nullcontext to preserve the outer bf16 autocast.
        # torch.autocast(enabled=False) would disable the outer autocast, causing dtype mismatches.
        adaln_ctx = torch.autocast(device_type=x_B_T_H_W_D.device.type, dtype=torch.float32) if use_fp32 else nullcontext()
        with adaln_ctx:
            if self.use_adaln_lora:
...
class Anima(nn.Module):
    ...
    def enable_block_swap(self, num_blocks: int, device: torch.device):
        ...
        self.offloader = custom_offloading_utils.ModelOffloader(self.blocks, self.num_blocks, self.blocks_to_swap, device)
