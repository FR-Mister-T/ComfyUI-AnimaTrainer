...
def main(args):
    ...
    for k in keys:
        if not args.reverse:
            ...
            # Module name conversion: convert underscores back to dots
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Anima LoRA between sd-scripts and ComfyUI formats")
    parser.add_argument(
        "src_path",
        type=str,
        default=None,
        help="source path, sd-scripts format (or ComfyUI compatible format if --reverse is set)",
    )
    parser.add_argument(
        "dst_path",
        type=str,
    ...
    parser.add_argument("--reverse", action="store_true", help="reverse conversion direction (ComfyUI -> sd-scripts)")
